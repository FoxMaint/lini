...
- **One syntax highlighter** — `lini::highlight_html` for a crate, `lini highlight` for a build step, `highlight()` for the browser build, and `lini highlight --css` for the palette all three wear. It reads the same word lists the editor grammars do and preserves every byte of the source, so a listing in a book, on a site, or in the playground is the same markup in the same colours.
